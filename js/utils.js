/* ============================================================
   Shared utilities — theme, toasts, guards, formatting
   ============================================================ */
...
function initials(name, email){
  ...
  return src.trim().split(/\s+/).map(p => p[0]).join("").slice(0,2).toUpperCase();
}

/* ---------- streak calculation ---------- */
function computeStreak(dates){
  // dates: array of ISO date strings (entry created_at), most recent first not required
  const daySet = new Set(dates.map(d => new Date(d).toDateString()));
  let streak = 0;
  let cursor = new Date();
  // allow today to be "not yet written" without breaking streak from yesterday
