/* ============================================================
   Profile — view/update profile, password, export, delete
   ============================================================ */

let profileUser = null;

(async function init(){
  profileUser = await renderShell(null);
...
async function saveProfile(e){
  ...
  const { error: authErr } = await supabaseClient.auth.updateUser({ data: { name } });
  const { error: profileErr } = await supabaseClient.from("profiles").upsert({ id: profileUser.id, name, email: profileUser.email, bio });

  if(authErr || profileErr){ toast((authErr || profileErr).message, true); return; }
...
async function exportJournal(){
  const { data: entries, error } = await supabaseClient
    .from("diary_entries").select("*").order("created_at", { ascending: true });
  if(error){ toast(error.message, true); return; }
  ...
  const text = entries.map(e => {
    return [
      ...
      `Date: ${formatDateLong(e.created_at)}`,
      `Category: ${e.category || "Personal"}`,
      `Mood: ${e.mood ? moodLabel(e.mood) : "—"}`,
      "",
      stripHtml(e.content || ""),
      "",
      "—".repeat(40),
  ...
  a.href = url;
  a.download = "inkwell-journal.txt";
  a.click();
...
async function deleteAccount(){
  const ok = await confirmModal({
    ...
    message: "This permanently deletes every entry, tag, and your profile. This can't be undone.",
